...
export default {
  ...
  data () {
    ...
    return {
      columns: [
        ...
        { title: '物流类别ID', key: 'logisticsTypeId', ellipsis: true, table: true, detail: true },
        { title: '物流类别名称', key: 'logisticsTypeName', ellipsis: true, table: true, detail: true },
  ...
  methods: {
    ...
    deleteRow (row) {
      return () => {
        this.$http.request({
          url: `/logisticsType/delete/${row.logisticsTypeId}`
    ...
    viewDetail (row, type) {
      return () => {
        this.$http.request({
          method: 'get',
          url: `/logisticsType/get/${row.logisticsTypeId}`
        }).then(res => {
          this.form = res.data
    ...
    onOk () {
      this.$http.request({
        url: `/logisticsType/${this.modal.type}`,
        data: this.form
      }).then(res => this.getTable())
    },
    pageChange (page) {
