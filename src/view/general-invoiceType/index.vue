...
export default {
  ...
  data () {
    ...
    return {
      columns: [
        ...
        { title: 'invoiceTypeId', key: 'invoiceTypeId', width: 120, ellipsis: true },
        { title: 'invoiceTypeName', key: 'invoiceTypeName', width: 150, ellipsis: true },
  ...
  methods: {
    ...
    deleteRow (row) {
      return () => {
        this.$http.request({
          url: `/invoiceType/delete/${row.goodsTypeId}`
    ...
    viewDetail (row) {
      return () => {
        this.$http.request({
          method: 'get',
          url: `/invoiceType/get/${row.goodsTypeId}`
        }).then(res => {
          this.form = res.data
