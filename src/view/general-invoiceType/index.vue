...
export default {
  ...
  data () {
    ...
    return {
      columns: [
        ...
        { title: 'invoiceTypeId', key: 'invoiceTypeId', width: 120, ellipsis: true, table: true, detail: true },
        { title: 'invoiceTypeName', key: 'invoiceTypeName', width: 150, ellipsis: true, table: true, detail: true },
  ...
  methods: {
    ...
    deleteRow (row) {
      return () => {
        this.$http.request({
          url: `/invoiceType/delete/${row.invoiceTypeId}`
    ...
    viewDetail (row, type) {
      return () => {
        this.$http.request({
          method: 'get',
          url: `/invoiceType/get/${row.invoiceTypeId}`
        }).then(res => {
          this.form = res.data
    ...
    onOk () {
      this.$http.request({
        url: `/invoiceType/${this.modal.type}`,
        data: this.form
      }).then(res => this.getTable())
    },
    pageChange (page) {
