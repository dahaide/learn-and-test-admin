<template>
  <div>
    ...
    <Modal v-model="modal.model" title="新增" @on-cancel="onCancel" @on-ok="onOk">
      <Form ref="form" :model="form" :label-width="120" :disabled="modal.disabled" style="padding: 0px 20px">
...
export default {
  ...
  data () {
    ...
    return {
      ...
      data: [],
      modal: { model: false, disabled: false },
      form: {}
    }
  },
  mounted () {
  ...
  methods: {
    ...
    deleteRow (row) {
      return () => {
        this.$http.request({
          url: `/user/delete/${row.goodsTypeId}`
